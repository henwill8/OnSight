...
const fetchJobStatus = async (jobId: string) => {
  try {
    const statusResponse = await fetchWithTimeout(`${config.API_URL}/api/job-status/${jobId}`, { method: "GET" }, 5000);
...
export const pollJobStatus = async (jobId: string, intervalLength: number, handleJobDone: any, handleJobError: any) => {
  try {
    let statusData = await fetchJobStatus(jobId);
    ...
    while (statusData.status !== "done" && statusData.status !== "error") {
      ...
      statusData = await fetchJobStatus(jobId); // Fetch new status data
    }
